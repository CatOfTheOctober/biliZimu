...
这个脚本测试修复后的 Python 版本是否能成功下载指定视频的 AI 字幕。
"""
...
def test_real_subtitle_download():
    """测试下载真实视频的 AI 字幕"""
    ...
    if bbdown_cookie_path.exists():
        try:
            ...
            logger.warning(f"无法读取 BBDown Cookie: {e}")
    else:
        logger.warning(f"BBDown Cookie 文件不存在: {bbdown_cookie_path}")
        logger.info("将使用无登录状态进行测试")
    ...
    try:
        ...
        # 检查是否获取成功（get_video_info 返回字典，不返回 success 字段）
        if not video_info or not video_info.get('aid'):
            logger.error(f"获取视频信息失败: {video_info}")
            return False
        
        logger.info(f"视频标题: {video_info.get('title')}")
        logger.info(f"视频 aid: {video_info.get('aid')}")
        logger.info(f"视频 cid: {video_info.get('cid')}")
        logger.info(f"视频页数: {video_info.get('page_count')}")
        ...
        if not aid or not cid or not bvid:
            logger.error(f"视频信息不完整: aid={aid}, cid={cid}, bvid={bvid}")
            return False
    ...
    # 使用新的统一字幕获取函数
    try:
        logger.info(f"开始获取 AI 字幕...")
        result = api.get_subtitle_with_ai_fallback(aid, cid, bvid)
        
        if not result.get('success'):
            logger.error(f"获取字幕失败: {result.get('message')}")
            return False
        
        logger.info(f"✅ 成功获取字幕!")
        logger.info(f"字幕语言: {result['metadata'].get('lan')}")
        logger.info(f"字幕描述: {result['metadata'].get('lan_doc')}")
        logger.info(f"字幕数量: {len(result['subtitles'])}")
        
        # 显示前 3 条字幕
        logger.info("前 3 条字幕:")
        for i, subtitle in enumerate(result['subtitles'][:3], 1):
            logger.info(f"  [{i}] {subtitle.get('from'):.1f}s - {subtitle.get('to'):.1f}s: {subtitle.get('content')}")
        ...
        logger.info(f"字幕文件大小: {output_file.stat().st_size} 字节")
...
if __name__ == '__main__':
    logger.info("=" * 80)
    logger.info("开始实际测试：下载真实视频的 AI 字幕")
    logger.info("=" * 80)
    
    success = test_real_subtitle_download()
